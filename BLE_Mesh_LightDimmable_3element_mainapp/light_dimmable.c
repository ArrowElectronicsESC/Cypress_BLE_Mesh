...
#define MESH_APP_NUM_MODELS_BLUE  (sizeof(mesh_element3_models) / sizeof(wiced_bt_mesh_core_config_model_t))


#define MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_RED     0
#define MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_GREEN   1
#define MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_BLUE    2
...
wiced_bt_mesh_core_config_element_t mesh_elements[] =
{
	    ...
	    },
	{
	    ...
	    .sensors_num = 0,                                               // Number of sensors in the sensor array
	    .sensors = NULL,                                                // Array of sensors of that element
	    .models_num = MESH_APP_NUM_MODELS_BLUE,                         // Number of models in the array models
...
void mesh_app_init(wiced_bool_t is_provisioned)
{
    ...
    wiced_bt_mesh_model_light_lightness_server_init(MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_RED, mesh_app_message_handler, is_provisioned);
    ...
    wiced_bt_mesh_model_light_lightness_server_init(MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_BLUE, mesh_app_message_handler, is_provisioned);

    // Initialize the Property Server.  We do not need to be notified when Property is set, because our only property is read only
...
void mesh_app_attention(uint8_t element_idx, uint8_t time)
{
    ...
    led_control_set_brighness_level(RED, attention_brightness);
}
...
void mesh_app_process_set_level(uint8_t element_idx, wiced_bt_mesh_light_lightness_status_t *p_status)
{
    ...
    {
    	WICED_BT_TRACE("Mesh App process set level - SET RED BRIGHTNESS \r\n");
    	led_control_set_brighness_level(RED, last_known_brightness);
    }

    if (element_idx == MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_GREEN)
    {
    	WICED_BT_TRACE("Mesh App process set level - SET GREEN BRIGHTNESS \r\n");
    	led_control_set_brighness_level(GREEN, last_known_brightness);
    }

    if (element_idx == MESH_LIGHT_LIGHTNESS_SERVER_ELEMENT_INDEX_BLUE)
    {
    	WICED_BT_TRACE("Mesh App process set level - SET BLUE BRIGHTNESS \r\n");
    	led_control_set_brighness_level(BLUE, last_known_brightness);
    }
